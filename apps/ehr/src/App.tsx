...
// All route-level pages are loaded lazily so each lands in its own chunk and a
// fresh tab only downloads the code for the page it opens.
const UnsolicitedResultsInbox = lazy(() =>
  import('./features/external-labs/pages/UnsolicitedResultsInbox').then((m) => ({
    default: m.UnsolicitedResultsInbox,
  }))
);
const UnsolicitedResultsMatch = lazy(() =>
  import('./features/external-labs/pages/UnsolicitedResultsMatch').then((m) => ({
    default: m.UnsolicitedResultsMatch,
  }))
);
...
const Claim = lazy(() => import('./rcm').then((m) => ({ default: m.Claim })));
const Claims = lazy(() => import('./rcm').then((m) => ({ default: m.Claims })));
