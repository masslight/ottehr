...
exports.Sidebar = exports.sidebarMenuIcons = void 0;
...
var StyledButton = (0, material_1.styled)(material_1.Button)(function (_a) {
    ...
    return (__assign({ display: 'flex', width: '100%', height: '42px', borderBottom: '1px solid #e0e0e0', alignItems: 'center', textDecoration: 'none', color: 'inherit', padding: '0', margin: '0', transition: 'background-color 0.3s', textTransform: 'none', '& .MuiListItemText-primary': {
            whiteSpace: 'nowrap',
            overflow: 'hidden',
            textOverflow: 'ellipsis',
        }, '& .MuiListItem-root:hover': {
            backgroundColor: colors_1.otherColors.sidebarItemHover,
        }, '&.Mui-disabled': {
            color: theme.palette.text.primary,
        } }, (isActive === 'true' && {
        color: theme.palette.primary.main,
        borderRight: "2px solid ".concat(theme.palette.primary.main),
        backgroundColor: (0, material_1.alpha)(theme.palette.primary.main, 0.05),
...
var Sidebar = function () {
    ...
    var _b = (0, useAppointment_1.useAppointment)(appointmentID), telemedData = _b.visitState, refetch = _b.refetch;
    ...
    var status = appointment && encounter ? (0, utils_1.getVisitStatus)(appointment, encounter) : undefined;
    var _c = (0, usePractitioner_1.usePractitionerActions)(encounter, 'end', utils_1.PRACTITIONER_CODINGS.Admitter), isEncounterUpdatePending = _c.isEncounterUpdatePending, handleUpdatePractitioner = _c.handleUpdatePractitioner;
    var assignedIntakePerformerId = encounter ? (0, utils_1.getAdmitterPractitionerId)(encounter) : undefined;
    var handleCompleteIntake = function () { return __awaiter(void 0, void 0, void 0, function () {
        var error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 6, , 7]);
                    if (!assignedIntakePerformerId) return [3 /*break*/, 4];
                    return [4 /*yield*/, handleUpdatePractitioner(assignedIntakePerformerId)];
                case 1:
                    _a.sent();
                ...
                case 3:
                    ...
                    return [3 /*break*/, 5];
                case 4:
                    (0, notistack_1.enqueueSnackbar)('Please select intake practitioner first', { variant: 'error' });
                    _a.label = 5;
    ...
    var generateMenuItems = function (routes) {
        return routes
            .map(function (route) { return ({
            text: route.text,
            icon: exports.sidebarMenuIcons[route.iconKey],
            ...
            .filter(function (item) { return item.visibility.has(interactionMode); });
    };
    var menuItems = generateMenuItems(Object.values(routesCSS_1.routesCSS)
        .filter(function (route) { return !route.isSkippedInNavigation; })
        .filter(function (route) { return route.path !== routesCSS_1.ROUTER_PATH.OTTEHR_AI || (chartData === null || chartData === void 0 ? void 0 : chartData.aiChat) != null; }));
    return (<material_1.Drawer variant="permanent" open={open} sx={{
            position: 'relative',
            width: open ? drawerWidth : function (theme) { return theme.spacing(7); },
            flexShrink: 0,
            '& .MuiDrawer-paper': {
                position: 'relative',
                width: open ? drawerWidth : function (theme) { return theme.spacing(7); },
      ...
      <DrawerHeader sx={__assign(__assign({ display: 'flex', padding: '0px' }, (open
            ? { justifyContent: 'end', paddingRight: '10px' }
            : { justifyContent: 'center', paddingRight: '0px' })), { borderBottom: '1px solid #e0e0e0' })}>
      ...
      <material_1.List sx={{ padding: '0px' }}>
        {menuItems.map(function (item) {
            var comparedPath = (item === null || item === void 0 ? void 0 : item.activeCheckPath) || item.to;
            return (<StyledButton isActive={location.pathname.includes(comparedPath).toString()} key={item.text} onClick={function () {
                    requestAnimationFrame(function () {
                        navigate(item.to);
                    });
                }}>
              <material_1.ListItem data-testid={data_test_ids_1.dataTestIds.sideMenu.sideMenuItem(item.to)} sx={{ width: '100%', height: 'inherit' }}>
                <material_1.ListItemIcon sx={{ minWidth: 40 }}>{item.icon}</material_1.ListItemIcon>
                <material_1.ListItemText primary={item.text} sx={{ opacity: open ? 1 : 0 }}/>
              </material_1.ListItem>
            </StyledButton>);
        })}
      </material_1.List>
      <br />
      <CompleteIntakeButton_1.CompleteIntakeButton isDisabled={!appointmentID || isEncounterUpdatePending || status !== 'intake'} handleCompleteIntake={handleCompleteIntake} status={status}/>
    </material_1.Drawer>);
};
