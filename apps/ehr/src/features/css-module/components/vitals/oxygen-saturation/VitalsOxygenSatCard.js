...
var VitalsOxygenSatCard = function (_a) {
    var _b;
    ...
    var _c = (0, react_1.useState)(''), oxySatValueText = _c[0], setOxySatValueText = _c[1];
    ...
    var _e = (0, react_1.useState)(false), isOxySatValidationError = _e[0], setOxySatValidationError = _e[1];
    var _f = (0, react_1.useState)(false), isCollapsed = _f[0], setIsCollapsed = _f[1];
    ...
    var isDisabledAddButton = !oxySatValueText || isOxySatValidationError;
    var latestOxySatValue = (_b = currentObs[0]) === null || _b === void 0 ? void 0 : _b.value;
    var handleSaveOxySatObservation = (0, react_1.useCallback)(function (oxySatValueText) { return __awaiter(void 0, void 0, void 0, function () {
        var oxySatValueNumber, observationMethod, vitalObs, _a;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    oxySatValueNumber = (0, helpers_1.textToOxygenSatNumber)(oxySatValueText);
                    if (!oxySatValueNumber)
                        return [2 /*return*/];
                    observationMethod = (0, utils_1.toVitalOxygenSatObservationMethod)(observationQualifier);
                    _b.label = 1;
                case 1:
                ...
                case 3:
                    _a = _b.sent();
                    (0, notistack_1.enqueueSnackbar)('Error saving oxygen saturation data', { variant: 'error' });
                    return [3 /*break*/, 5];
                case 4:
                    setIsSaving(false);
                    return [7 /*endfinally*/];
    ...
    var handleTextInputChange = (0, react_1.useCallback)(function (e) {
        var oxySatAsText = e.target.value;
    ...
    var renderQualifierDropdown = function () {
        return (<material_1.FormControl fullWidth size="small" sx={{ backgroundColor: 'white' }} disabled={isSaving}>
        ...
        <material_1.Select value={observationQualifier} label="Qualifier" labelId="qualifier-label" variant="outlined" error={false} defaultValue="" onChange={function (event) {
          ...
          {Object.values(utils_1.VitalsOxygenSatObservationMethod).map(function (obsMethod) { return (<material_1.MenuItem key={obsMethod} value={obsMethod}>
              <material_1.Typography color="textPrimary" sx={{ fontSize: '16px' }}>
                {obsMethod}
              </material_1.Typography>
            </material_1.MenuItem>); })}
    ...
    return (<material_1.Box sx={{ mt: 3 }}>
      <components_1.AccordionCard label={"Oxygen sat (%) ".concat(latestOxySatValue !== null && latestOxySatValue !== void 0 ? latestOxySatValue : '')} collapsed={isCollapsed} onSwitch={handleSectionCollapse}>
        <components_1.DoubleColumnContainer divider leftColumn={<material_1.Grid container sx={{
              ...
              <material_1.Grid item xs={12} sm={4} md={4} lg={4} order={{ xs: 1, sm: 1, md: 1 }}>
                <VitalsTextInputFiled_1.VitalsTextInputFiled label="Sat (%)" value={oxySatValueText} disabled={isSaving} isInputError={isOxySatValidationError} onChange={handleTextInputChange}/>
              </material_1.Grid>

              {/* Qualifier/method dropdown column */}
              <material_1.Grid item xs={12} sm={4} md={4} lg={4} order={{ xs: 2, sm: 2, md: 2, lg: 2 }} sx={{ mt: isLargeScreen ? 0 : 0 }}>
                <material_1.Box sx={{
                display: 'flex',
                flexDirection: 'row',
                ml: 1,
              ...
              {/* Add Button column */}
              <material_1.Grid item xs={12} sm={4} md={4} lg={4} order={{ xs: 3, sm: 3, md: 3, lg: 3 }} sx={{ mt: 0 }}>
                <RoundedButton_1.RoundedButton disabled={isDisabledAddButton} loading={isSaving} onClick={function () { return handleSaveOxySatObservation(oxySatValueText); }} color="primary" sx={{
                height: '40px',
                px: 2,
                ml: 1,
            }}>
...
exports.default = VitalsOxygenSatCard;
//# sourceMappingURL=VitalsOxygenSatCard.js.map