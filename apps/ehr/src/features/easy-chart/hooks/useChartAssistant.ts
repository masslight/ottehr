  ...
  const runTurn = useCallback(
    async (message: string): Promise<void> => {
      ...
      try {
        ...
        // ONE pass, no split. apply-template only SUGGESTS a template — the server resolves its id and the
        // provider applies it by hand from the picker — so nothing lands mid-plan that the rest of the
        // plan would have to re-read the chart for or reconcile against.
