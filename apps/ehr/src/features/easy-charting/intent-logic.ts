...
// Vital readings a message states, matched by UNIT keyword so an unrelated number ("5 days of cough",
// "seen 3 times") is never counted as one. Used only to COUNT readings for routing; parsing them is
// the zambdas' job (normalizeVitalIntent).
...
export function countVitalReadings(msg: string): number {
  return VITAL_READING_PATTERNS.filter((re) => re.test(msg)).length;
}

// Which path a typed message takes: the PLANNER (narrative → ordered steps) when it's long, spans
// several sentences, or states MORE THAN ONE vital reading; otherwise the single-shot AGENT.
//
// The multi-reading rule exists because the agent returns exactly ONE action. "patient is 5'8",
// weighs 130lb" is 30 characters and one sentence, so it took the agent path and could chart at most
// one of the two vitals — the other was dropped with nothing said about it. Two readings need two
// steps, which is the planner's job.
export function looksLikeNarrative(msg: string): boolean {
  if (msg.length >= 140) return true;
  // Sentence-end punctuation followed by whitespace/EOL — a couple of sentences in a short message is
  // a narrative; a single declarative isn't.
  const sentenceEnds = msg.match(/[.!?](?:\s|$)/g);
  if ((sentenceEnds?.length ?? 0) >= 2) return true;
  return countVitalReadings(msg) >= 2;
}
