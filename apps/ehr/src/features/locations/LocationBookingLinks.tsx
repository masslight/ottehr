...
import { Alert, Box, Button, Link as MuiLink, Skeleton, Tooltip, Typography } from '@mui/material';
...
import { useLocationSchedulesQuery } from './location.queries';
...
// Schedule has no `name` in R4B; the display-name extension is the only human label it carries, and
// it's optional. Fall back to a positional label so a second, unnamed schedule is still tellable
// apart from the first.
const scheduleLabel = (schedule: Schedule, index: number): string =>
  schedule.extension?.find((ext) => ext.url === SCHEDULE_DISPLAY_NAME_EXTENSION_URL)?.valueString ??
  `Schedule ${index + 1}`;
 ...
 * booked.
 *
 * Owning the Schedule lookup is what lets this warn instead of handing out a dead link: a Location
 * with no Schedule still produces syntactically valid prebook URLs that resolve and then vend nothing.
 */
export function LocationBookingLinks({ location }: LocationBookingLinksProps): ReactElement {
  ...
  const { data: schedules, isLoading } = useLocationSchedulesQuery(location.id);
        ...
        <>
          ...
          {slug && (schedules ?? []).length === 0 && (
            <Alert severity="warning" sx={{ mb: 2 }} data-testid={dataTestIds.locationConfig.noScheduleWarning}>
              ...
              Create a schedule to make it bookable.
            </Alert>
          )}
