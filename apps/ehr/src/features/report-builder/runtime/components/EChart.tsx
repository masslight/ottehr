...
export function EChart({ option, height = 400, onClick }: EChartProps): React.ReactElement {
  ...
  // Create the chart ONCE. Re-initializing on every option change (a click updates parent state, so
  // `option` is a fresh object each render) would dispose + recreate the chart and reset the user's
  // zoom/pan/selection. Option updates instead go through the effect below via setOption, which keeps
  // that view state — a click must never rescale or re-settle the chart.
  useEffect(() => {
    ...
    chart.on(
      ...
      (params: { name?: unknown; value?: unknown; seriesName?: unknown; dataIndex?: unknown; data?: unknown }) => {
        ...
        // Forward a consistent datum for EVERY data shape. ECharts gives the category name, the value,
        // the series and the point index even when series data is plain numbers (the common case).
        // Object-form data points are spread on top so their own fields are available too.
        const base = {
          name: params.name,
          value: params.value,
  ...
  // Apply the option (initial + every update). replaceMerge on series swaps the data cleanly while
  // preserving the current zoom/scale and other view state (dataZoom defaults carry no start/end, so
  // the merge keeps the user's range).
  useEffect(() => {
    ...
    try {
      ...
      chart.setOption(full as echarts.EChartsCoreOption, { replaceMerge: ['series'] });
      setError(null);
    } catch (e) {
