...
import VitalsNotesCard from 'src/components/patient-info/VitalsNotesCard';
import VitalsBloodPressureCard from 'src/components/vitals/blood-pressure/VitalsBloodPressureCard';
import VitalsHeartbeatCard from 'src/components/vitals/heartbeat/VitalsHeartbeatCard';
import VitalsHeightCard from 'src/components/vitals/heights/VitalsHeightCard';
import { useDeleteVitals } from 'src/components/vitals/hooks/useDeleteVitals';
import { useGetHistoricalVitals, useGetVitals } from 'src/components/vitals/hooks/useGetVitals';
import { useSaveVitals } from 'src/components/vitals/hooks/useSaveVitals';
import VitalsOxygenSatCard from 'src/components/vitals/oxygen-saturation/VitalsOxygenSatCard';
import VitalsRespirationRateCard from 'src/components/vitals/respiration-rate/VitalsRespirationRateCard';
import VitalsTemperaturesCard from 'src/components/vitals/temperature/VitalsTemperaturesCard';
import VitalsVisionCard from 'src/components/vitals/vision/VitalsVisionCard';
import VitalsWeightsCard from 'src/components/vitals/weights/VitalsWeightsCard';
...
export const VitalsTab: FC = () => {
  ...
  if (isLoading || isChartDataLoading || encounterVitalsLoading) return <Loader />;
  if (error) return <Typography>Error: {error.message}</Typography>;
  if (!appointment) return <Typography>No data available</Typography>;

  return (
    <Stack spacing={1}>
      <PageTitle label="Vitals" showIntakeNotesButton={true} />
