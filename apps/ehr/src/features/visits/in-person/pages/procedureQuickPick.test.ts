...
describe('procedure quick pick data transformations', () => {
  it('keeps saved CPTs as selections without using them to route the family', () => {
    const target: ProcedurePageState = {};
    applyProcedureQuickPick(
      target,
      {
        name: 'Unknown procedure',
        cptCodes: [{ code: '12001', display: 'Laceration repair' }],
        infusionStartTime: '09:00',
        infusionStopTime: '10:00',
      },
      'Unknown procedure'
    );
    expect(target.cptCodes?.[0].code).toBe('12001');
    expect(target.infusionStartTime).toBe('09:00');
  });
  ...
  it('builds the persisted quick pick from the current procedure state', () => {
    const quickPick = buildProcedureQuickPick(
      ...
      '  Procedure preset  ',
      'Laceration Repair (Wound Closure)',
      [{ name: 'Laceration Repair (Wound Closure)', code: 'laceration-repair' }]
    );
