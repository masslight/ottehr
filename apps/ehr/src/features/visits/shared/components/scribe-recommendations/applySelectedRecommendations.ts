...
/**
 * Applies every selected, not-yet-applied recommendation in the panel store, one at a time, and
 * records each outcome on the row. Sequential on purpose: the per-row status then reads as a
 * checklist filling in, and later items see what earlier ones wrote (a primary diagnosis, a
 * template's diagnoses). `reconcile` runs once at the end regardless of failures.
 */
export const applySelectedRecommendations = async (
  ...
  const toApply = sortForApply(
    store.recommendations.filter((rec) => {
      const item = store.itemState[rec.id];
