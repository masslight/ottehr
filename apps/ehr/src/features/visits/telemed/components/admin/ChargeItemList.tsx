...
export default function FeeSchedule({ mode = 'fee-schedule' }: FeeScheduleProps): ReactElement {
  ...
  const { data: feeScheduleData, isPending: fsPending } = useListFeeSchedulesQuery({ enabled: !isChargeMaster });
  const { data: chargeMasterData, isPending: cmPending } = useListChargeMastersQuery({ enabled: isChargeMaster });
