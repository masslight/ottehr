...
import { BILLING_URL, INSURANCES_URL } from 'src/App';
...
export default function InsuranceNotesList(): ReactElement {
  ...
  const { data: ehrInsuranceOverrideList } = useQuery({
  ...
  const payerIds = (
    ehrInsuranceOverrideList?.entry?.map((entry) => extractPayerIdFromUrl(entry.item.reference)) ?? []
  ...
  const filteredInsurances = useMemo(() => {
      ...
      .filter((insurance: Organization) => {
        ...
        if (
          ehrInsuranceOverrideList?.entry
            ...
            ?.extension?.find((e) => e.url === ottehrExtensionUrl('insurance-override-note'))
    ...
    return newData || [];
  }, [data, ehrInsuranceOverrideList, searchText]);
    ...
    <Paper sx={{ padding: 2, marginTop: 2 }}>
      ...
      >
        ...
        <Typography variant="body2" color="inherit">
          Insurance notes are displayed in the Payments section on the Visit Details page of the EHR. This feature
          allows you to leave notes for front-desk staff about specific payers.
        </Typography>
      </Box>
      <TableContainer>
        <Grid container spacing={2} display="flex" alignItems="center">
          <Grid item xs={12} sm={5}>
            <TextField
              fullWidth
              id="outlined-basic"
              label="Search by name or note content..."
              onChange={(e) => {
          ...
          <Grid item xs={12} sm={3} display={'flex'}>
            <Link to={`${BILLING_URL}/insurance/notes/new`} style={{ width: '100%' }}>
              <Button
                sx={{
                  borderRadius: 100,
                  textTransform: 'none',
              ...
              >
                <AddIcon />
                <Typography fontWeight="bold">Add to notes list</Typography>
              </Button>
            </Link>
          </Grid>
        </Grid>
        ...
        <Table sx={{ minWidth: 650 }} aria-label="insurancesTable">
          ...
          <TableHead>
            <TableRow>
              ...
              <TableCell sx={{ fontWeight: 'bold', width: '40%' }}>Insurance Note</TableCell>
          ...
          <TableBody>
            ...
            {currentPagesEntities.map((insurance: Organization, idx: number) => {
              ...
              const insuranceNote = ehrInsuranceOverrideList?.entry
                ?.find((e) => e.item.reference === getPayerUrl(getPayerId(insurance) ?? ''))
                ?.extension?.find((e) => e.url === ottehrExtensionUrl('insurance-override-note'))?.valueString;
              const payerId = getPayerId(insurance);
              return (
                <TableRow key={idx}>
                  <TableCell>
                    <Link
                      to={`${INSURANCES_URL}/notes/${insurance.id}`}
                      style={{
                        display: 'contents',
                  ...
                  <TableCell>{insuranceNote ?? <Skeleton width={100} height="100%" />}</TableCell>
