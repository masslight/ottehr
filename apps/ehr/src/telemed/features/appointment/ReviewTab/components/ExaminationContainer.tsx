...
export const ExaminationContainer: FC<ExaminationContainerProps> = (props) => {
  ...
  const noRashesItem = examObservationFieldsDetailsArray
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          <ExamReviewGroup
            label="Skin:"
            items={
              noRashesItem.length ? noRashesItem : [{ label: 'Rashes', abnormal: true } as ExamObservationFieldItem]
            }
          />

          {rashesLabels.length > 0 && <Typography>{rashesLabels.join(', ')}</Typography>}

          <ExamReviewComment item={examObservations['skin-comment']} />
        </Box>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          <ExamReviewGroup
            ...
            items={examObservationFieldsDetailsArray
              .map((details) =>
                details.group === 'dropdown' ? { ...details, label: `Tender ${details.label}` } : details
              )
              .filter((details) => details.card === 'abdomen')
              .filter((details) => examObservations[details.field].value)}
          />

          <ExamReviewComment item={examObservations['abdomen-comment']} />
        </Box>
