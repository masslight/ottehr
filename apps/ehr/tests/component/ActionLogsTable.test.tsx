...
describe('ActionLogsTable', () => {
  ...
  it('dashes out a fax whose sending organization has no number on file', async () => {
    ...
    // Located via the header rather than by counting dashes, since other columns dash out too.
    const senderColumn = screen.getAllByRole('columnheader').findIndex((header) => header.textContent === 'Sender Fax');
    expect(senderColumn).toBeGreaterThan(-1);
    expect(screen.getAllByRole('cell')[senderColumn]).toHaveTextContent('-');
  });
