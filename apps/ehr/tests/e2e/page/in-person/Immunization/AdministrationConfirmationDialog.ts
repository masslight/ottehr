...
export class AdministeredDialogue {
  ...
  async selectReason(reason: string): Promise<void> {
    ...
    // Scope to the listbox option: the same reason text also appears in MAR reason cells for orders
    // left by earlier tests on a shared appointment, so a page-wide getByText would match several
