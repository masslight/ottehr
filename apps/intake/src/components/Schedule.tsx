...
import { createLocalDateTime, DATE_FULL_NO_YEAR, DATETIME_FULL_NO_YEAR, nextAvailableFrom } from 'utils';
    ...
    if (!slotsLoading)
      return (
        <Alert severity="error">
          {t('schedule.officeClosed.todayOrTomorrow', `${window.APP_CONFIG?.projectName ?? ''}`)}
        </Alert>
      );
    return <></>;
  }
            ...
            <Box>
              <TabPanel value={currentTab} index={0} dir={theme.direction}>
                ...
                <SelectSlot
                  ...
                  noSlotsMessage={
                    slotsExist
                      ? t('schedule.officeClosed.today', `${window.APP_CONFIG?.projectName ?? ''}`)
                      : t('schedule.officeClosed.todayOrTomorrow', `${window.APP_CONFIG?.projectName ?? ''}`)
                  }
                />
              </TabPanel>
