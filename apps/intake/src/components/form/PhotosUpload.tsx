...
import { filterObject, findMissingNumber } from 'ui-components/lib/helpers';
import { MultipleFileUploadOptions } from 'ui-components/lib/types';
...
export const PhotosUpload: FC<PatientPhotoUploadProps> = ({ name, label, defaultValue = {}, options }) => {
  ...
  const [images, setImages] = useState(() => {
    return Object.keys(defaultValue).reduce(
      (prev, curr) => {
        const url = defaultValue?.[curr]?.localUrl || defaultValue?.[curr]?.presignedUrl;
        if (url) {
          prev[curr] = url;
        }
        return prev;
