...
export const useCardAutofill = ({ items, pageId }: UseCardAutofillInput): void => {
  const { appointment, questionnaireResponse } = usePaperworkContext();
  ...
  // query key matches the primary carrier field's useAnswerOptionsQuery key (its top-level
  // fieldId), so react-query shares the cache with the rendered field rather than refetching.
  ...
  const { data: payerOptions } = useAnswerOptionsQuery(
    INSURANCE_CARRIER_LINK_ID,
    Boolean(carrierAnswerSource) && anyInsuranceTrigger,
    carrierAnswerSource ? { answerSource: carrierAnswerSource } : undefined
  );
