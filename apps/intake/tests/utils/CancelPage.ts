...
import { CancellationReasonOptionsInPerson, CancellationReasonOptionsTelemed } from 'utils';
...
export class CancelPage {
  ...
  async selectCancellationReason(serviceMode: 'in-person' | 'virtual'): Promise<void> {
    const cancelOptions =
      serviceMode === 'in-person' ? CancellationReasonOptionsInPerson : CancellationReasonOptionsTelemed;
