...
#   config {
#     distribution_id = var.billing_cloudfront_distribution_id
#     paths           = ["/*"]
#   }
# }
...
resource "terraform_data" "billing_upload" {
  ...
  provisioner "local-exec" {
    command = "aws s3 sync ${path.module}/../../../../apps/billing/build s3://${var.billing_bucket_id} --profile ${var.aws_profile} --delete --exact-timestamps && aws s3 cp ${path.module}/../../../../apps/billing/build/index.html s3://${var.billing_bucket_id}/index.html --profile ${var.aws_profile} --cache-control 'no-cache, must-revalidate'"
  }
