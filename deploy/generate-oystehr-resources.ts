...
async function generate(): Promise<void> {
    ...
    if (!schemaVersion || !(typeof schemaVersion === 'string') || !validSchemas.hasOwnProperty(schemaVersion)) {
...
async function generate20250319(specs: { path: string, spec: {[key: string]: any} }[], vars: {[key: string]: any}, outputPath: string): Promise<void> {
    ...
    for (const [appName, app] of Object.entries(resources.apps)) {
        appResources.resource.oystehr_application[appName] = {
            ...
            allowed_callback_urls: getValue(app.allowedCallbackUrls, vars, resources),
...
function validateSpec20250319(specFile: { path: string, spec: { [key: string]: unknown } }): ValidSpec20250319 {
    ...
    for (const key of Object.keys(spec)) {
        if (!['schema-version', 'apps', 'buckets', 'fhirResources', 'labRoutes', 'm2ms', 'roles', 'secrets', 'zambdas'].includes(key)) {
            throw new Error(`${specFile.path} has unknown top-level key: ${key}`);
        }
    }
    if (!['apps', 'buckets', 'fhirResources', 'labRoutes', 'm2ms', 'roles', 'secrets', 'zambdas'].some(key => spec.hasOwnProperty(key))) {
        throw new Error(`${specFile.path} must have at least one of the following top-level keys: apps, buckets, fhirResources, labRoutes, m2ms, roles, secrets, zambdas.`);
    }
...
function getValue(value: any, vars: { [key: string]: any }, resources: ValidSpec20250319): any {
    ...
    const refReplacedValue = varReplacedValue.replace(/\#\{ref\/([^\}/]+)\/([^\}/]+)\/([^\}]+)\}/g, (match: string, resourceType: string, resourceName: string, fieldName: string) => {
        if (isResourceType(resourceType) && resources[resourceType].hasOwnProperty(resourceName)) {
...
function isResourceType(resourceType: string): resourceType is keyof ValidSpec20250319 {
    return ['apps', 'buckets', 'fhirResources', 'labRoutes', 'm2ms', 'roles', 'secrets', 'zambdas'].includes(resourceType);
}
