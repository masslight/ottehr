...
resource "local_sensitive_file" "zambda_secrets_for_local_server" {
  count    = var.is_local ? 1 : 0
  content  = jsonencode(var.zambda_secrets_for_local_server)
  filename = "${path.module}/../../packages/zambdas/.env/zambda-secrets-${var.environment}.json"
}

resource "terraform_data" "zambda_secrets_for_local_server_format" {
  count            = var.is_local ? 1 : 0
  triggers_replace = local_sensitive_file.zambda_secrets_for_local_server[0].content_md5
  provisioner "local-exec" {
    command = "node -e \"const fs=require('fs'),f=process.argv[1];fs.writeFileSync(f,JSON.stringify(JSON.parse(fs.readFileSync(f,'utf8')),null,2))\" '${local_sensitive_file.zambda_secrets_for_local_server[0].filename}'"
  }
}
