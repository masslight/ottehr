...
merged PDF — the *fax packet*:

```
[cover sheet] + [Visit/Progress Note] + [Discharge Summary] + [Lab Results] + [Radiology Results] + [Patient Education]
```

Merge order is fixed (`FAX_DOCUMENT_ORDER`), independent of the order the user checked things in.

| Document | Source | Available when |
|---|---|---|
...
**Prescriptions and Patient Instructions are sections of the visit note**, not standalone documents. The
selector shows them as disabled child rows of "Visit/Progress Note" and they travel with it.
...
Delivery reuses the existing outbound-delivery machinery: one `Task` per recipient, `oystehr.fax.send`, and
the `Communication` it returns. Everything shows up in the patient's Action Logs (Fax Logs tab) and in the visit's activity log, with
per-recipient retry. The Task records the recipient's organization and phone, the packet page count and the
list of documents that went into it.
...
- One recipient failing does not abort the others; `SendFaxPacketOutput.results` reports each one and the UI
  lists the failures in a dialog.
- Saving a recipient as the patient's PCP never fails the send — it comes back as `pcpSaveError`.
- Processing is synchronous. `send-fax-packet` runs with 3008 MB and the 27 s timeout ceiling API Gateway
  imposes on `http_auth` zambdas (it merges up to 20 MB of PDFs in memory).
...
| Zambdas | `packages/zambdas/src/ehr/{send-fax-packet,get-fax-packet-preview}` |
...
`useForm` from it at mount (no syncing effect); `useSendFax` owns the dialog lifecycle and the mutation.
`DocumentSelector` is deliberately fax-agnostic so the planned "Discharge & More" screen can reuse it for
fax, email and print.
