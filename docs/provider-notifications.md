...
12 task categories) and, per type, chooses a delivery method, a set of locations, and an "assigned to"
filter.
...
Preferences are stored as a **single JSON string** inside a child of the existing settings-extension
container (same convention as `ScheduleExtension` / QuickPick config). Legacy flat values are written
**alongside** the V2 blob so any code still reading the old shape keeps working.

```
Practitioner.extension[]
└─ url: https://fhir.ottehr.com/r4/provider-notifications-settings      (container)
   ├─ url: .../r4/provider-notifications-method            valueString  "Phone and Computer"   (legacy, derived)
   ├─ url: .../r4/provider-notifications-enabled-task       valueBoolean true                   (legacy, derived)
   ├─ url: .../r4/provider-notifications-enabled-telemed    valueBoolean true                   (legacy, derived)
   └─ url: https://fhir.ottehr.com/r4/provider-notifications-preferences-v2
                                                            valueString  "<JSON.stringify(V2)>"  (source of truth)

Practitioner.telecom[]  →  { system: 'sms', value: '+1XXXXXXXXXX' }   (phone number for SMS)
```

- **Source of truth:** the V2 JSON blob. The three legacy children are *derived* on every save (see §6).
...
interface ProviderNotificationPreferencesV2 {
  ...
  taskCategories: Record<UiTaskCategoryId, NotificationRowPref>;  // the 12 task-category rows
}
```
...
### 2.3 Task categories (12 UI rows)
...
> `task-assigned` is emitted by the **assignment engine** (§4.3). For migrated staff it delivers all
...
time. `'me'` rows (and later assignments) are delivered by the assignment engine (§4.3), so they aren't
...
- The **booking** path keeps the original `location.address.state` guard. The **waiting-room** path does
  NOT require a location state: the previous check-in delivery (the waiting-room Task fan-out) never
...
path in §4.2 is the sole check-in emitter — this prevents double-notifying a single check-in).
...
| --- | --- | --- |
| **Task notifications** | Legacy `task-assigned` only — **tasks assigned to them**, gated on legacy `taskNotificationsEnabled`. Category engine **ignores** them. | `task-category-created` at creation for `'anyone'` rows; `task-assigned` from the assignment engine for `'me'` rows and for later assignments (§4.3). The two engines coordinate so one event never notifies the same person twice. |
...
| **Legacy readers** | Unaffected. | Still valid: on save we also write derived `method` (`rollupLegacyNotificationMethod`) + `taskNotificationsEnabled`/`telemedNotificationsEnabled` (`deriveLegacyNotificationFlagsFromV2`). |
...
**Derivation on save** (`useUpdateProviderNotificationPreferencesV2Mutation`):

```
taskNotificationsEnabled  = any taskCategories[*].enabled
telemedNotificationsEnabled = waitingRoom.enabled || virtualVisitScheduled.enabled
method (legacy rollup)     = both phone & computer among enabled rows ⇒ 'Phone and Computer'
                             else any phone ⇒ 'Phone'; else any computer ⇒ 'Computer'; else 'Phone and Computer'
```
...
appointments the old cron already handled (§4.2, deploy transition).
...
| Task assigned **after** creation | The creation round is over (task already tagged), so the assignment engine notifies the new owner — for `'me'` rows *and* `'anyone'` rows (§4.3). Reassignments after the first `task-assigned` tag do not re-fire (pre-existing single-shot behavior). |
