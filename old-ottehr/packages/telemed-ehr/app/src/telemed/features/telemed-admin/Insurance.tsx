...
export default function Insurances(): ReactElement {
  ...
  const [statusFilter, setStatusFilter] = React.useState<IsActiveStatus | undefined>(undefined);
  ...
  const filteredInsurances = React.useMemo(
    () =>
      (searchText
        ? data?.filter((insurance: InsurancePlan) => {
            insurance.name?.[0]?.toLowerCase().includes(searchText.toLowerCase()) &&
  ...
  const handleStatusChange = (ev: SelectChangeEvent<IsActiveStatus | undefined>): void => {
    setStatusFilter(ev.target.value as IsActiveStatus | undefined);
  };
      ...
      <TableContainer>
        <Grid container spacing={2} display="flex" alignItems="center">
          ...
          <Grid item xs={12} sm={2} display={'flex'}>
            <Link to="/telemed-admin/insurances/new" style={{ width: '100%' }}>
              <Button
                sx={{
                  borderRadius: 100,
                  textTransform: 'none',
            ...
            {currentPagesEntities.map((insurance: InsurancePlan, idx: number) => {
              ...
              return (
                <TableRow key={idx}>
                  <TableCell>
                    <Link
                      to={`/telemed-admin/insurances/${insurance.id}`}
                      style={{
                        display: 'contents',
...
function insuranceStatusCheck(curentStatus: IsActiveStatus | undefined, insurance: InsurancePlan): boolean {
  if (curentStatus === undefined) return true;
