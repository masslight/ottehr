  ...
  return new Promise((resolve, reject) => {
    fetch(`${projectApiUrl}/application`, {
      ...
      body: JSON.stringify({
        ...
        description: 'Example',
        loginRedirectUri: 'https://127.0.0.1:3200/dashboard',
        allowedCallbackUrls: ['http://localhost:3200', 'http://localhost:3200/dashboard'],
        allowedLogoutUrls: ['http://localhost:3200'],
        allowedWebOriginsUrls: ['http://localhost:3200'],
        allowedCORSOriginsUrls: ['http://localhost:3200'],
      }),
    })
      .then(async (response) => {
...
function createFrontEndLocalEnvFile(clientId: string, projectId: string): string {
  const envTemplatePath = 'packages/ehr/front-end/env/.env.local-template';
  const envPath = 'packages/ehr/front-end/env/.env.local';
  ...
  const updatedData = templateData
    .replace('VITE_APP_ZAPEHR_APPLICATION_CLIENT_ID=placeholder_client_id', `VITE_APP_ZAPEHR_APPLICATION_CLIENT_ID=${clientId}`);
