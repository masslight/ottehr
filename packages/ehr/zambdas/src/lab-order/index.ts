...
import { getPatchBinary, IntakeQuestionnaireItem, makeValidationSchema } from 'utils';
import { checkOrCreateM2MClientToken, createOystehrClient } from '../shared/helpers';
import { ZambdaInput } from 'zambda-utils';
import { validateRequestParameters } from './validateRequestParameters';
import { Provenance, Questionnaire, QuestionnaireItem, QuestionnaireResponseItemAnswer } from 'fhir/r4b';
...
export const index = async (input: ZambdaInput): Promise<APIGatewayProxyResult> => {
  try {
    ...
    const questionnaireItems: QuestionnaireItem[] = Object.keys(data).map((questionResponse) => {
