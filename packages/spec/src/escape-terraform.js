...
exports.escapeTerraformTemplateSyntax = escapeTerraformTemplateSyntax;
/**
 ...
 */
function escapeTerraformTemplateSyntax(value, terraformResourcePrefixes) {
    if (terraformResourcePrefixes === void 0) { terraformResourcePrefixes = ['oystehr_']; }
    ...
    var escaped = value.replace(/(\$+)\{/g, function (match, dollars, offset) {
        // Check what comes immediately after the {
        var afterBrace = value.slice(offset + match.length);
        // Check if this is a valid Terraform reference (starts with known prefix)
        var isValidReference = terraformResourcePrefixes.length > 0 && terraformResourcePrefixes.some(function (prefix) { return afterBrace.startsWith(prefix); });
        // Check if this is a Terraform function call (e.g., replace(...), join(...), etc.)
        var isFunctionCall = afterBrace.match(/^[a-zA-Z_][a-zA-Z0-9_]*\s*\(/) !== null;
    ...
    escaped = escaped.replace(/(%+)\{/g, function (_, percents) { return percents.replace(/%/g, '%%') + '{'; });
    return escaped;
}
