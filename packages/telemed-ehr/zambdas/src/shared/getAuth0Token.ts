...
export async function getAuth0Token(secrets: Secrets | null, type = 'regular'): Promise<string> {
  ...
  if (type === 'regular') {
    AUTH0_CLIENT = getSecret(SecretsKeys.URGENT_CARE_AUTH0_CLIENT, secrets);
    AUTH0_SECRET = getSecret(SecretsKeys.URGENT_CARE_AUTH0_SECRET, secrets);
  } else if (type === 'messaging') {
    AUTH0_CLIENT = getSecret(SecretsKeys.URGENT_CARE_MESSAGING_M2M_CLIENT, secrets);
    AUTH0_SECRET = getSecret(SecretsKeys.URGENT_CARE_MESSAGING_M2M_SECRET, secrets);
  } else {
  ...
  console.log(AUTH0_AUDIENCE, AUTH0_CLIENT, AUTH0_SECRET);
