...
import { checkTelemedVirtualLocations } from './setup-default-locations';
...
async function createApplication(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (patientRole) {
      ...
      console.log(`${projectApiUrl}/iam/roles/${patientRole.id}`);
      const patientRoleRes = await fetch(`${projectApiUrl}/iam/role/${patientRole.id}`, {
        method: 'PATCH',
        headers: {
          accept: 'application/json',
          'content-type': 'application/json',
    ...
    fetch(`${projectApiUrl}/application`, {
      ...
      body: JSON.stringify({
        ...
        allowedCallbackUrls: ['http://localhost:3002', 'http://localhost:3002/patients'],
...
function createZambdaLocalEnvFile(
  ...
  const overrideData = {
    ...
    TELEMED_MESSAGING_DEVICE_ID: m2mDeviceId,
