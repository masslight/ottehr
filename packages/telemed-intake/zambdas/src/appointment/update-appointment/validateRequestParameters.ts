...
import { PersonSex, RequiredProps, Secrets, UpdateAppointmentRequestParams, ZambdaInput } from 'ottehr-utils';
...
export function validateUpdateAppointmentParams(
  ...
  const { patient, appointmentId, unconfirmedDateOfBirth } = JSON.parse(input.body);
