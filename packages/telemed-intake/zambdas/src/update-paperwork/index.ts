...
import { getPatientResource, getQuestionnaireResponse, updatePatientResource } from '../shared/fhir';
...
import { PaperworkResponse } from 'ottehr-utils/lib/types/data/paperwork';
...
export const index = async (input: ZambdaInput): Promise<APIGatewayProxyResult> => {
  ...
  try {
    ...
    const { appointmentID, paperwork, paperworkComplete, ipAddress, files } = validateUpdatePaperworkParams(
      input,
      questionnaire,
    );
    console.groupEnd();
    ...
    if (hipaa && consentToTreat) {
      await checkAndCreateConsent(questionnaireResponseResource, paperwork, patientResource, appointmentID, fhirClient);
    }
...
async function createCardsResourcesAndPDFs(
  ...
  const photoIdCardFrontUrl = files?.['photo-id-front']?.z3Url;
  const photoIdCardBackUrl = files?.['photo-id-back']?.z3Url;
