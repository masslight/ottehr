...
// Test ids preserved from intake's `dataTestIds` so existing E2E selectors keep matching.
const ITEM_ADDED_VALUE_TEST_ID = 'item-added-value';
const DELETED_BUTTON_TEST_ID = 'delete-button';

// Inlined `delete.svg` (intake's `deleteIcon`) as a data URI so this component is self-contained
// and renders the identical 18px trash icon without a cross-package asset import.
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5, mb: 3 }}>
      ...
      {listValue.map((item: SelectedItem, index: number) => (
        ...
        >
          <Typography data-testid={ITEM_ADDED_VALUE_TEST_ID}>{`${item?.valueString}${
          ...
          >
            <img alt="delete icon" src={DELETE_ICON_SRC} width={18} data-testid={DELETED_BUTTON_TEST_ID} />
          </IconButton>
        </Box>
      ))}
    </Box>
