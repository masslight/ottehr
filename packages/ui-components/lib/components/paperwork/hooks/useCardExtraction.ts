...
export const useCardExtraction = <T extends ExtractableCardDocumentFileType>(
  ...
  const enabled = Boolean(
    paperworkComponentHelpers.getCardExtraction !== undefined && appointmentID && trigger != null
  );
  ...
  const { data } = useQuery({
    ...
    queryFn: () => paperworkComponentHelpers.getCardExtraction!({ appointmentID: appointmentID!, cardType }),
    enabled,
