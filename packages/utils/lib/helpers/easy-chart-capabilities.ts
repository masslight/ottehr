...
export const EASY_CHART_CAPABILITIES = {
  ...
  'add-allergy': { surfaces: ['agent', 'planner'], chartField: 'allergies', required: ['display'] },
  ...
  'add-condition': { surfaces: ['agent', 'planner'], chartField: 'conditions', required: ['display'] },
  // NOT offered to review either: its med-name and med-reconcile checks deliberately correct the NOTE
  // TEXT rather than the order ("We CANNOT create the eRx order programmatically" — they emit
  // edit-note-text and set partial:true). remove-medication IS offered, for the coherence check.
  'add-medication': { surfaces: ['agent', 'planner'], chartField: 'medications', required: ['display'] },
  'add-surgical-history': { surfaces: ['agent', 'planner'], chartField: 'surgicalHistory', required: ['display'] },
  'add-hospitalization': { surfaces: ['agent', 'planner'], chartField: 'episodeOfCare', required: ['display'] },
  'add-diagnosis': { surfaces: ['agent', 'planner', 'review'], chartField: 'diagnosis', required: ['display'] },

  // Removes — matched against what is already on this chart.
  'remove-allergy': { surfaces: ['agent', 'planner'], chartField: 'allergies', required: ['display'] },
  'remove-condition': { surfaces: ['agent', 'planner'], chartField: 'conditions', required: ['display'] },
  'remove-medication': { surfaces: ['agent', 'planner', 'review'], chartField: 'medications', required: ['display'] },
  'remove-surgical-history': { surfaces: ['agent', 'planner'], chartField: 'surgicalHistory', required: ['display'] },
  'remove-hospitalization': { surfaces: ['agent', 'planner'], chartField: 'episodeOfCare', required: ['display'] },
  'remove-diagnosis': { surfaces: ['agent', 'planner', 'review'], chartField: 'diagnosis', required: ['display'] },

  // Billing codes — the model supplies the code directly.
  'set-em-code': { surfaces: ['agent', 'planner', 'review'], chartField: 'emCode', required: ['code'] },
  'add-cpt': { surfaces: ['agent', 'planner', 'review'], chartField: 'cptCodes', required: ['code'] },
  ...
  'remove-em-code': { surfaces: ['agent', 'planner'], chartField: 'emCode', required: [] },
  'remove-cpt': { surfaces: ['agent', 'planner', 'review'], chartField: 'cptCodes', required: ['code'] },
  ...
  // list carried.
  'remove-exam-finding': { surfaces: ['agent', 'planner'], chartField: 'examObservations', required: ['display'] },
  // Polarity rides in `display` ("Denies …" / "Reports …"); `finding` is only a secondary signal.
  'add-ros-finding': { surfaces: ['agent', 'planner', 'review'], chartField: 'rosObservations', required: ['display'] },
  'remove-ros-finding': { surfaces: ['agent', 'planner'], chartField: 'rosObservations', required: ['display'] },
  ...
  'add-patient-instruction': { surfaces: ['agent', 'planner'], chartField: 'instructions', required: ['text'] },
  ...
  'set-disposition': {
    ...
    required: ['dispositionType'],
  },
  // `display` carries the reading the client parses the numbers out of, so it is load-bearing.
  'set-vital': { surfaces: ['agent', 'planner'], chartField: 'vitalsObservations', required: ['field', 'display'] },
...
export function easyChartKindsForSurface(surface: EasyChartSurface): EasyChartIntentKind[] {
  return EASY_CHART_INTENT_KINDS.filter((kind) => (CAPABILITIES[kind].surfaces as readonly string[]).includes(surface));
}
...
export function easyChartIntentHasRequiredFields(kind: string, raw: Record<string, unknown>): boolean {
  const capability = CAPABILITIES[kind as EasyChartIntentKind];
