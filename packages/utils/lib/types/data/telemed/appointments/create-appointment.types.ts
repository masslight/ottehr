...
export type PatientInfo = PatientBaseInfo & {
  ...
  /**
   * Screening question answers captured on the booking form for the in-person
   * flow (keyed by FHIR field id). Routed through create-appointment into the
   * in-person paperwork QR so the existing harvest pipeline turns them into
   * Observations.
   */
