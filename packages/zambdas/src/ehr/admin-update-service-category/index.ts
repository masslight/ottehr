...
const validateRequestParameters = (input: ZambdaInput): AdminUpdateServiceCategoryInput => {
  ...
  // `name: undefined`, empty type[] coding). The admin UI always sends the
  // complete record loaded from the catalog, so requiring all fields is
  // contract-safe.
  if (!serviceCategory || typeof serviceCategory !== 'object')
    throw INVALID_INPUT_ERROR('"serviceCategory" must be an object');

  const missing: string[] = [];
  if (!serviceCategory.id) missing.push('serviceCategory.id');
