...
const ZAMBDA_NAME = 'complete-encounters-report';

export const index = wrapHandler(ZAMBDA_NAME, async (input: ZambdaInput): Promise<APIGatewayProxyResult> => {
  let validatedParameters: CompleteEncountersReportZambdaInput & { secrets: Secrets };
  try {
    ...
    let pageCount = 1;
    console.log(`Fetching page ${pageCount} of complete encounters...`);
    ...
    // Filter encounters to only include those in the completed terminal state
    ...
    console.log(`Found ${completeEncounters.length} complete encounters`);

    // Build the response data
    const encounterItems = completeEncounters.map((encounter) => {
      const appointmentRef = encounter.appointment?.[0]?.reference;
      const appointment = appointmentRef ? appointmentMap.get(appointmentRef) : undefined;
    ...
    const response: CompleteEncountersReportZambdaOutput = {
      message: `Found ${encounterItems.length} complete encounters`,
