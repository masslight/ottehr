...
import { createOystehrClient, createPresignedUrl, topLevelCatch, wrapHandler, ZambdaInput } from '../../shared';
...
export const index = wrapHandler(ZAMBDA_NAME, async (input: ZambdaInput): Promise<APIGatewayProxyResult> => {
  try {
    ...
    const presignedFileDownloadUrl = await createPresignedUrl(input.accessToken!, z3URL, 'download');
