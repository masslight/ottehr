...
export const index = async (input: ZambdaInput): Promise<APIGatewayProxyResult> => {
  ...
  try {
    ...
    const _oystehrCurrentUser = createOystehrClient(validatedParameters.userToken, validatedParameters.secrets);
    const _practitionerIdFromCurrentUser = await getMyPractitionerId(_oystehrCurrentUser);
