...
// "vital-respiration-rate" → "respiration rate", for messages the provider reads.
const label = (field: string): string => field.replace(/^vital-/, '').replace(/-/g, ' ');

// What to try when a vital couldn't be read. Deliberately phrased as EXAMPLES, not as the accepted
// set — the server converts any unit in its table (cm/m/mm, in/ft, kg/g, lb/oz/st), so listing a few
// familiar forms helps without implying the rest are unsupported.
const hint = (field: string): string => {
  const examples: Record<string, string> = {
    'vital-temperature': '"temp 100.4 F" or "38 C"',
    'vital-heartbeat': '"heart rate 88"',
    'vital-respiration-rate': '"respiratory rate 18"',
    'vital-oxygen-sat': '"O2 sat 97%"',
    'vital-weight': '"130 lb", "59 kg", or "9 lb 4 oz" — any weight unit works',
    'vital-height': `"5'8\\"", "68 inches", "173 cm", or "1.73 m" — any length unit works`,
  };
  return `Try ${examples[field] ?? '"temp 100.4 F"'}.`;
};
...
export // Exported for the capability-coverage test (it asserts the prompt documents every action kind the
// agent surface offers).
...
- "set-vital": record a vital sign. Triggered by "temp 102.2 F", "add a temperature of 100.4",
  "heart rate 88", "BP 122 over 78", "O2 sat 97%", "respiratory rate 18", "add height 5.8 inches",
  "weighs 130lb", "patient is 1.73 m", "9 lb 4 oz". HEIGHT and WEIGHT are ordinary vitals — never
  answer "unknown" for them, and never reject a reading because of the unit it is written in.
  DO NOT convert, round, or normalize the number, and DO NOT restrict yourself to any particular set
  of units: copy the reading into "display" VERBATIM, exactly as the provider wrote it, whatever unit
  that is (cm, m, mm, inches, feet, feet+inches, kg, g, lb, lb+oz, stones, …). The server owns unit
  conversion and will tell the provider if a unit is unrecognized — a verbatim copy is always the
  right answer, a converted one may be wrong. Required
  ...
  if (i.kind === 'unknown') {
    ...
    if (i.kind === 'set-em-code' || i.kind === 'add-cpt') {
      ...
      if (typeof i.field !== 'string' || !(VITAL_FIELDS as readonly string[]).includes(i.field)) {
        ...
        } else if (typeof i.implausible === 'string') {
          // The number was understood but is not a height a person can have — almost always decimal
          // feet written as inches. Name the ambiguity and both valid formats instead of guessing
          // which one was meant, or (worse) charting a 15 cm patient.
          intent = {
            kind: 'unknown',
            message:
              `A height of ${i.implausible} isn't possible for a person, so I didn't chart it — I'd be ` +
              `guessing which unit you meant. ${hint('vital-height')}`,
          };
        } else if (typeof i.unrecognizedUnit === 'string') {
          // The number was there but its unit isn't one we can convert. Say which unit, rather than
          // assuming a default and charting a wrong number.
          intent = {
            kind: 'unknown',
            message: `I don't recognize "${i.unrecognizedUnit}" as a unit for ${label(i.field)}. ${hint(i.field)}`,
          };
        } else {
          // Name the vital that failed and show that vital's formats — the old generic "I need a value,
          // e.g. set temp to 100.4 F" was useless when the provider was entering a height.
          intent = {
            kind: 'unknown',
            message: `I couldn't read a ${label(i.field)} value from that. ${hint(i.field)}`,
          };
        }
      }
    } else if (!display) {
