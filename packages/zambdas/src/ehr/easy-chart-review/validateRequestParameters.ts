import { EasyChartNoteContext, EasyChartReviewInput, INVALID_INPUT_ERROR, MISSING_REQUIRED_PARAMETERS } from 'utils';
...
export function validateRequestParameters(input: ZambdaInput): EasyChartReviewInput & Pick<ZambdaInput, 'secrets'> {
  ...
  const { narrative, noteContext, chartState, encounterId } = parsed as Record<string, unknown>;
