 ...
 * every nested searchset into one resource list, and follows `next` links with follow-up batches. A failing
  ...
  while (pending.length > 0 && round <= MAX_NEXT_PAGE_ROUNDS) {
    ...
    results.forEach((batchBundle, batchIndex) => {
      ...
      (batchBundle.entry ?? []).forEach((entry, entryIndex) => {
        ...
        if (next) nextUrls.push(toBatchRelativeUrl(next, options.fhirApiUrl));
      });
    });
