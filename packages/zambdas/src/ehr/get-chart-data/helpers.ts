...
/**
 * Scopes a patient-scoped search to the subject of an encounter without needing the patient's id:
 * `?subject:Patient._has:Encounter:subject:_id=<encounterId>`. Resolving the patient up front cost a
 * full serialized FHIR round trip in front of every chart load; this removes the need for it.
 *
 * Verified against the direct `?subject=Patient/<id>` form for every patient-scoped resource type
 * this endpoint searches, using two patients seeded with identical-looking resources: each
 * encounter's search returned exactly its own patient's resource and never the other patient's.
 *
