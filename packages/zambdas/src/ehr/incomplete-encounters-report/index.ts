...
export const index = wrapHandler(ZAMBDA_NAME, async (input: ZambdaInput): Promise<APIGatewayProxyResult> => {
  ...
  // This fetches all appointments, encounters, patients, locations, and practitioners without
  // being subject to the 6MB response-size limit or pagination timeouts.
  const allResources = await searchAllFhirAsync<Appointment | Encounter | Patient | Location | Practitioner>(oystehr, {
