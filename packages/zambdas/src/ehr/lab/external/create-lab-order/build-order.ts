...
export interface BuildExternalLabOrderRequestsInput {
  oystehr: Oystehr;
  dx: DiagnosisDTO[];
  encounter: Encounter;
...
/**
 * Builds the FHIR transaction requests that create external lab orders for the
 * given orderable items: per test, a draft ServiceRequest with its pre-submission
 * "Collect sample" Task, Provenance, Specimens (unless PSC), AOE
 * QuestionnaireResponse, and clinical-info Communication. Orders are grouped into
 * requisitions by lab, reusing an existing requisition number when a bundle-able
 * draft order already exists on the encounter.
 *
 * Shared between the create-lab-order zambda (provider ordering from the chart)
 * and apply-template (materializing a global template's external lab plans), so
 * the order shape stays consistent between the two writers. The caller is
 * responsible for executing the returned requests in a FHIR transaction.
 */
export async function buildExternalLabOrderRequests(
  input: BuildExternalLabOrderRequestsInput
