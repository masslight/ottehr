...
export function validateRequestParameters(input: ZambdaInput): MakeMedicationHistoryPdfZambdaInput & { secrets: any } {
  ...
  const { patient, appointment, medicationHistory, encounter } = JSON.parse(input.body);
  ...
  return { patient, appointment, medicationHistory, encounter, secrets: input.secrets };
}
