...
let m2mToken: string;
export const index = wrapHandler('activate-employer', async (input: ZambdaInput): Promise<APIGatewayProxyResult> => {
  try {
    ...
    // Sync activation to Candid (best-effort)
    const candid = createCandidClientIfConfigured(secrets);
    if (candid) {
      const candidPayerId = getCandidPayerIdFromOrganization(existing);
      if (candidPayerId) {
        await syncToggleCandidEmployerPayer(candid, candidPayerId, true);
      } else {
        console.log(`[activate-employer] No Candid payer ID on Organization ${employerId} — skipping Candid toggle`);
      }
    }
    ...
    return topLevelCatch('activate-employer', error, ENVIRONMENT);
  }
});
