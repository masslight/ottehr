...
let m2mToken: string;
export const index = wrapHandler('associate-location', async (input: ZambdaInput): Promise<APIGatewayProxyResult> => {
  const { feeScheduleId, locationId, secrets } = validateRequestParameters(input);
  ...
  // Check if this location is already associated
