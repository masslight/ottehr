// Reusable core of the easy-chart planner: prompt building, template-title fetch, the structured
// LLM invoke (with acceptResult validation + escalation), and all post-parse normalization.
// Extracted from the easy-chart-planner zambda so the ambient-scribe pipeline can precompute a
// plan server-side; the zambda's handler is now a thin wrapper around runEasyChartPlanner.
// The prompt bytes are PINNED by tests (easy-chart-dx-selection-pins.test.ts) — do not reword.
...
import { derivePatientStatus, fetchPatientContext } from './patient-context';
import { detectSpeakerLabels, recoverSourceText, sniffDoseFormScoped, sniffIcdCodeScoped } from './sniffers';
import { normalizeVitalIntent, sniffVitalsFromNarrative } from './vitals';
...
// The planner pipeline shared by the easy-chart-planner zambda and the ambient-scribe precompute:
// fetch templates/patient context (best-effort), build the pinned prompt, invoke the structured
// chatbot with retry/escalation, then normalize/validate the steps. `oystehr` may be undefined
// (client init failed upstream) — the planner still degrades gracefully without templates/patient.
export async function runEasyChartPlanner(
  params: EasyChartPlannerInput,
  oystehr: Oystehr | undefined,
  secrets: Secrets | null
): Promise<EasyChartPlannerOutput> {
  const { narrative, noteContext, chartState, encounterId, incremental } = params;
