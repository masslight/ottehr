...
export async function buildFaxPacketSection(args: {
  ...
  const partBytes = await Promise.all(
    parts.map(async (part) => {
      try {
        if (part.bytes) return await normalizeFileToPdf(part.bytes, part.contentType);
        if (!part.z3Url) {
          throw new Error('has neither generated bytes nor a stored file');
        }
        const bytes = await downloadFileBytes(part.z3Url, token);
        return await normalizeFileToPdf(bytes, part.contentType ?? getMimeType(part.z3Url));
      } catch (error) {
        const reason = error instanceof Error ? error.message : 'could not be rendered';
        throw new Error(`Fax packet part "${part.title}" ${reason}. The entire fax was not sent.`, { cause: error });
      }
    })
  );
