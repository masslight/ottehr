...
describe('detectDispositionLanguage', () => {
  describe('positive disposition language', () => {
    it.each([
      ...
      ['returning-if', 'Plan is returning if the fever climbs past 103 or he stops drinking.'],
      ['returning-if', 'Returning when the swelling is down so we can recheck the range of motion.'],
      ['back-here', 'I want him back here same day if he gets worse.'],
      ['back-here', 'Back here tomorrow for a wound check.'],
      ['back-here', 'She should be back here in 3 days if the rash spreads.'],
  ...
  describe('negations and non-disposition look-alikes', () => {
    it.each([
      ...
      ['past-tense recurrence', 'The hives came back the next morning after the shower.'],
      ['returning to activity', 'He is returning to work on Monday and returning to practice next week.'],
      ['symptom keeps returning', 'The pain keeps returning at night despite the ibuprofen.'],
      ['back here with no time', 'Mom says they were back here last month for the same ear.'],
