...
describe('createConsentResources', () => {
  const [SOLE_FORM] = getConsentFormsForLocation();
  ...
  test('creates, uploads, and files one PDF per configured consent form', async () => {
    ...
    // One PDF per form in the reference config (single combined HIPAA + consent-to-treat form)
    expect(mockCreatePdfBytes).toHaveBeenCalledTimes(1);
    const pdfInfo = mockCreatePdfBytes.mock.calls[0][3];
    expect(pdfInfo.formTitle).toEqual(SOLE_FORM.formTitle);
    expect(pdfInfo.copyFromPath).toBe(SOLE_FORM.assetPath);

    // Upload URL keyed by project bucket, patient, timestamp, and form id
    ...
    // The sole form creates a Consent resource, linked to its docref
    ...
    expect(consentDocRefId).toBe(`dr-${SOLE_FORM.type.text}-0`);
  ...
  test('resolves consent form assets for the configured location state', async () => {
    await run({ location: makeLocation('IL') });
    const pdfInfo = mockCreatePdfBytes.mock.calls[0][3];
    expect(pdfInfo.copyFromPath).toBe(IL_FORMS[0].assetPath);
  });
  ...
  test('wraps upload failures with the form title', async () => {
    ...
    await expect(run()).rejects.toThrow(`Failed to upload ${SOLE_FORM.formTitle} PDF. z3 unavailable`);
  });
  ...
  test('throws when no DocumentReference matches the consent form title', async () => {
    ...
    await expect(run()).rejects.toThrow(`DocumentReference for "${SOLE_FORM.formTitle}" not found`);
  });
});
