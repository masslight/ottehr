...
// Mock getConsentFormsForLocation with a fixed 2-form config (HIPAA + consent-to-treat)
// so tests are independent of project overlay, which may vary per deployment.
vi.mock('utils/lib/ottehr-config/consent-forms', async (importOriginal) => {
  ...
  return {
    ...
    getConsentFormsForLocation: (state?: string) =>
      state === 'IL' ? [HIPAA_MOCK, CTT_IL_MOCK] : [HIPAA_MOCK, CTT_DEFAULT_MOCK],
  };
});
