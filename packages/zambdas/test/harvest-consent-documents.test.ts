...
describe('createConsentResources', () => {
  ...
  test('creates, uploads, and files one PDF per configured consent form', async () => {
    ...
    const formsCreatingConsent = ALL_FORMS.filter((f) => f.createsConsentResource);
    expect(mockCreateConsentResource).toHaveBeenCalledTimes(formsCreatingConsent.length);
