...
// Pin the two-form default config so this test is independent of instance-specific overlays
// that may collapse HIPAA and CTT into a single combined form.
vi.mock('utils/lib/ottehr-config/consent-forms', async (importOriginal) => {
  ...
  return {
    ...
    getConsentFormsForLocation: (locationState?: string) =>
      locationState === 'IL' ? [hipaaForm, cttFormIL] : [hipaaForm, cttFormDefault],
  };
});
