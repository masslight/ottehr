...
// Provide a stable two-form config (HIPAA + CTT) so tests are independent of the
// project-level overlay that may configure fewer forms in production.
vi.mock('utils/lib/ottehr-config/consent-forms', async (importOriginal) => {
  ...
  return {
    ...
    getConsentFormsForLocation: (locationState?: string) =>
      locationState === 'IL' ? [HIPAA, CTT_IL] : [HIPAA, CTT_DEFAULT],
  };
});
