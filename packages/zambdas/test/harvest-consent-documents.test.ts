...
describe('createConsentResources', () => {
  ...
  test('creates, uploads, and files one PDF per configured consent form', async () => {
    ...
    expect(mockCreatePdfBytes).toHaveBeenCalledTimes(ALL_FORMS.length);
    const pdfInfos = mockCreatePdfBytes.mock.calls.map((call) => call[3]);
    expect(pdfInfos.map((info) => info.formTitle)).toEqual(ALL_FORMS.map((f) => f.formTitle));
    expect(pdfInfos[1].copyFromPath).toBe(ALL_FORMS[1].assetPath);
    ...
    expect(mockUploadPDF).toHaveBeenCalledTimes(ALL_FORMS.length);
    expect(mockUploadPDF).toHaveBeenCalledWith(
      expect.any(Uint8Array),
      `${expectedBase}-${ALL_FORMS[0].id}.pdf`,
      'test-token',
      PATIENT_ID
    );
    ...
    const consentForms = ALL_FORMS.filter((f) => f.createsConsentResource);
  ...
  test('resolves state-specific consent form assets (the Illinois variant)', async () => {
    const ilVariantIndex = ALL_FORMS.findIndex((f, i) => IL_FORMS[i].assetPath !== f.assetPath);
    if (ilVariantIndex === -1) {
      // No IL-specific form variants configured in the current consent forms setup
      return;
    }
    await run({ location: makeLocation('IL') });
    const ilPdfInfo = mockCreatePdfBytes.mock.calls[ilVariantIndex][3];
    expect(ilPdfInfo.copyFromPath).toBe(IL_FORMS[ilVariantIndex].assetPath);
    expect(ilPdfInfo.copyFromPath).not.toBe(ALL_FORMS[ilVariantIndex].assetPath);
  });
  ...
  test('throws when no DocumentReference matches the consent form title', async () => {
    ...
    const firstConsentForm = ALL_FORMS.find((f) => f.createsConsentResource)!;
