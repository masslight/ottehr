...
describe('createConsentResources', () => {
  const [HIPAA_FORM, CTT_FORM, CREDIT_CARD_FORM] = getConsentFormsForLocation();
  ...
  test('creates, uploads, and files one PDF per configured consent form', async () => {
    ...
    // One PDF per form in the reference config (HIPAA + consent-to-treat + credit-card-agreement)
    expect(mockCreatePdfBytes).toHaveBeenCalledTimes(3);
    ...
    expect(mockUploadPDF).toHaveBeenCalledTimes(3);
    ...
    // Both consent-to-treat and credit-card-agreement forms create Consent resources
  ...
  test('resolves state-specific consent form assets (the Illinois variant)', async () => {
    ...
    // IL_FORMS resolves state-conditional paths; no IL-specific override is configured so the
    // default CTT path is used, but the resolver is exercised and the correct form is selected.
