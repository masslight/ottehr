...
describe('createConsentResources', () => {
  ...
  test('creates, uploads, and files one PDF per configured consent form', async () => {
    ...
    const consentCreatingForms = allForms.filter((f) => f.createsConsentResource);
    ...
    // One Consent resource per form that declares createsConsentResource, linked to its docref
  ...
  test('resolves state-specific consent form assets (the Illinois variant)', async () => {
    ...
    // Only assert the IL path differs from the default when the config actually defines state-specific paths
