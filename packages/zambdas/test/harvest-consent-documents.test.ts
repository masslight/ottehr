...
describe('createConsentResources', () => {
  ...
  test('creates, uploads, and files one PDF per configured consent form', async () => {
    ...
    // Each form that declares createsConsentResource: true produces exactly one Consent resource
    const formsWithConsent = getConsentFormsForLocation().filter((f) => f.createsConsentResource);
    expect(mockCreateConsentResource).toHaveBeenCalledTimes(formsWithConsent.length);
    // The CTT form's Consent is linked to its matching DocumentReference by title
    const cttCall = mockCreateConsentResource.mock.calls.find((args) => args[1] === `dr-${CTT_FORM.type.text}-0`);
    expect(cttCall).toBeDefined();
    const [consentPatientId, consentDocRefId, consentDate] = cttCall!;
  ...
  test('resolves state-specific consent form assets (the Illinois variant)', async () => {
    ...
    // This assertion only holds when the config provides a state-specific IL override
