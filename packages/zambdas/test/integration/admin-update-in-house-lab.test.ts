...
// Happy path for admin-update-in-house-lab: toggle-status flips a lab definition's
// status (active<->retired) in place. Creates a throwaway lab, deletes it in teardown.
describe('admin-update-in-house-lab integration — happy path', () => {
  ...
  beforeAll(async () => {
    const setup = await setupIntegrationTest('admin-update-in-house-lab.test.ts', M2MClientMockType.provider);
  ...
  it('toggles the in-house lab status', async () => {
    const response = await oystehrProvider.zambda.execute({
      id: 'admin-update-in-house-lab',
      userId: randomUUID(),
      data: { updateType: 'toggle-status', data: { activityDefinitionId } },
    });
