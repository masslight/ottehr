...
// Happy path for ehr-get-visit-details: returns the EHR visit detail bundle for an
// appointment. Requires an intake QuestionnaireResponse, so the seed QR is pointed
// at the instance's active intake questionnaire (resolved from config).
describe('ehr-get-visit-details integration — happy path', () => {
  ...
  beforeAll(async () => {
    const setup = await setupIntegrationTest('ehr-get-visit-details.test.ts', M2MClientMockType.provider);
  ...
  it('returns the EHR visit details for an appointment', async () => {
