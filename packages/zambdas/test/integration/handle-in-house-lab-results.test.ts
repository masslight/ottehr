...
// Happy path for handle-in-house-lab-results: enter results for an in-house lab
// order whose specimen has been collected. Full chain: define lab -> order ->
// collect specimen -> enter results. Result data is keyed by the AD's contained
// ObservationDefinition ids. FHIR-only.
describe('handle-in-house-lab-results integration — happy path', () => {
  ...
  beforeAll(async () => {
    const setup = await setupIntegrationTest('handle-in-house-lab-results.test.ts', M2MClientMockType.provider);
    ...
    resultsData = {};
    (ad.contained ?? [])
      .filter((r): r is ObservationDefinition => r.resourceType === 'ObservationDefinition')
      .forEach((od) => {
        if (od.id) resultsData[od.id] = 'Negative';
      });
  ...
  it('enters results for an in-house lab order', async () => {
    expect(Object.keys(resultsData).length).toBeGreaterThan(0);
