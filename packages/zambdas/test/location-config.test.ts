...
describe('location config matches spec', () => {
  ...
  test('in-person locations match', () => {
    const sortedInPersonConfig = [...LOCATION_CONFIG.inPersonLocations.map((loc: { name: string }) => loc.name)].sort();
  ...
  test('telemed locations match', () => {
    const sortedTelemedConfig = [...LOCATION_CONFIG.telemedLocations.map((loc: { name: string }) => loc.name)].sort();
