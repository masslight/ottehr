...
// Read from VALUE_SETS so this test adapts to project overlays that may clear this list.
const PER_ENCOUNTER_CPT_CODE = VALUE_SETS.externalLabCptCodesToAddPerEncounter[0]?.value;
...
describe('collectExternalLabCptProcedures', () => {
  ...
  test.skipIf(!VALUE_SETS.externalLabCptCodesToAddPerEncounter?.length)(
  ...
  describe.skipIf(!VALUE_SETS.externalLabCptCodesToAddPerEncounter?.length)(
    'per-encounter CPT code (e.g. 99001)',
    () => {
