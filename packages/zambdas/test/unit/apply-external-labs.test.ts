...
// Undefined when the overlay opts out of per-encounter billing (empty array).
const effectivePerEncounterCode = VALUE_SETS.externalLabCptCodesToAddPerEncounter?.[0]?.value;
...
describe('collectExternalLabCptProcedures', () => {
  ...
  test.skipIf(!effectivePerEncounterCode)(
    ...
    async () => {
      for (const paymentMethod of [
        ...
        expect(codes).toContain(effectivePerEncounterCode);
  ...
  describe('per-encounter CPT code (e.g. 99001)', () => {
    test.skipIf(!effectivePerEncounterCode)(
      'adds the per-encounter code once when a matched plan is non-PSC',
      async () => {
        ...
        const perEncounterProcedures = result.procedures.filter(
          (p) => p.code?.coding?.some((c) => c.code === effectivePerEncounterCode)
        );
        expect(perEncounterProcedures).toHaveLength(1);
        expect(result.cptCodesToSkip).toContain(effectivePerEncounterCode);
      }
    );
    ...
    test.skipIf(!effectivePerEncounterCode)(
      'adds the per-encounter code only once across multiple non-PSC matched plans',
      async () => {
