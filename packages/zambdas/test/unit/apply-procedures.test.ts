...
describe('collectContainedIdsClaimedByProcedures', () => {
  ...
  test('returns an empty set when proceduresAction is skip', () => {
    // No plans apply, so no contained resources need to be force-created on
    // their behalf. Mirrors collectCptCodesFromApplicableActivityDefinitions
    // returning an empty set when its sections are skipped.
