...
describe('makeCreateRequests — procedure plans', () => {
  ...
  // Helper builds the linked-procedure template fixture used by the
  // skip-override tests: a Dx Condition + a CPT Procedure on the template,
  // plus a procedure plan that links to both.
  ...
  test("procedures='append' + diagnoses='skip': procedure's linked Dx is force-created and added to encounter.diagnosis", () => {
    ...
    // so it shows in the chart's standalone Diagnoses list too. The patch op
    // could be 'add' (new) or 'replace' (replacing existing); narrow before
    // touching .value (RemoveOperation doesn't carry one).
  ...
  test("procedures='skip': claimedByProcedures is empty, standalone Dx skip really skips the Dx", () => {
    ...
    // skip-override doesn't kick in - this confirms the helper returns an
    // empty Set in that case and the standalone Dx behavior is unchanged.
    const templateList = makeLinkedProcedureTemplate();
