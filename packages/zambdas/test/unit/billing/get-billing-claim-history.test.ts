...
describe('get-billing-claim-history performEffect', () => {
  ...
  it('skips a malformed change set instead of failing the whole request', async () => {
    ...
    // Pass a non-local ENVIRONMENT so sendErrors doesn't bail on its local-env guard.
    const { entries } = await performEffect(oystehr, { claimId: 'c1', secrets: { ENVIRONMENT: '' } as any });
