...
describe('cm-add-procedure-code - validateRequestParameters', () => {
  ...
  test('should return validated params with optional fields', () => {
    const input = createMockZambdaInput(
      { chargeMasterId: validChargeMasterId, code: '99213', description: 'Office visit', modifier: '25', amount: 200 },
      { secrets }
    );
    const result = validateRequestParameters(input);
    expect(result).toEqual({
  ...
  test('should throw when chargeMasterId is not a valid UUID', () => {
    const input = createMockZambdaInput({ chargeMasterId: 'not-a-uuid', code: '99213', amount: 150 }, { secrets });
    expect(() => validateRequestParameters(input)).toThrow();
  });

  test('should throw when code is missing', () => {
    const input = createMockZambdaInput({ chargeMasterId: validChargeMasterId, amount: 150 }, { secrets });
    expect(() => validateRequestParameters(input)).toThrow();
  });

  test('should throw when amount is missing', () => {
    const input = createMockZambdaInput({ chargeMasterId: validChargeMasterId, code: '99213' }, { secrets });
    expect(() => validateRequestParameters(input)).toThrow();
  });

  test('should throw when amount is not a number', () => {
    const input = createMockZambdaInput(
      { chargeMasterId: validChargeMasterId, code: '99213', amount: 'notanumber' },
      { secrets }
    );
    expect(() => validateRequestParameters(input)).toThrow();
  });
