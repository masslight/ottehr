...
describe('send-fax - validateRequestParameters', () => {
  ...
  test('should prepend +1 to the fax number', () => {
    const input = createMockZambdaInput({ appointmentId: 'appt-123', faxNumber: '5551234567' }, { secrets });
    const result = validateRequestParameters(input);
  ...
  test('should throw when Authorization header is missing', () => {
    const input = createMockZambdaInput(
      { appointmentId: 'appt-123', faxNumber: '2125551234' },
  ...
  test('should throw when faxNumber is not a valid phone number', () => {
    const input = createMockZambdaInput({ appointmentId: 'appt-123', faxNumber: '123' }, { secrets });
    expect(() => validateRequestParameters(input)).toThrow();
  });
});
