...
describe('composeProcedures — structured fields', () => {
  ...
  it('preserves labeled answers, modifiers and quantity in the PDF data', () => {
    const composed = composeFor({
      procedureType: 'Burn Treatment / Dressing',
    ...
    expect(composed.procedures?.[0].structuredFacts).toBe(
      'Deepest burn degree treated: partial thickness\nTreated partial-thickness body surface (%): 5'
    );
    expect(composed.procedures?.[0].cptCodes).toEqual(['16025-LT × 2 — Burn care']);
  });
