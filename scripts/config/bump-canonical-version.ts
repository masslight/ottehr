...
async function main(): Promise<void> {
  try {
    ...
    const toBump = Object.entries(data.fhirResources).filter(
      ([key, resource]) => resource.resource && resource.resource.version && (!maybeKey || key === maybeKey)
    );
    if (toBump.length === 0) {
      console.error('The provided file must at least one fhirResource with a version field');
      process.exit(1);
    }
    ...
    for (const [resourceKey, originalResourceValue] of toBump) {
      ...
      if (archiveData.fhirResources[resourceKey].resource.status) {
