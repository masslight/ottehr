...
> ⚠ **Practitioners must also have an Oystehr IAM role assignment**
...
| `create-demo-user.ts` | Provision a `demo@ottehr.com` Administrator user via the Oystehr IAM invite flow + `changePassword`. Use this once per fresh synth project so there's at least one role-assigned Practitioner for the synth pipeline to attribute work to. |
| `create-synth-m2m.ts` | Provision a new M2M client on the target project with `profile: Practitioner/<existing-id>`, copying the access policy from a reference M2M. Use this on any project whose existing M2M client has a `Device/...` profile (the default for newly-created M2Ms) — without a Practitioner profile, `save-chart-data` fails 500 with `"Failed to get Practitioner: ... 404"` because the zambda blindly treats the profile id as a Practitioner id. The script needs IAM scope `App:CreateM2MClient`; the project's default M2M usually doesn't have it, so run this script using a Terraform-class M2M's credentials (e.g., `~/development/secrets/ottehr/terraform/<env>.tfvars`). After execution it prints the new clientId + secret (one-time only) — drop those into both `packages/zambdas/.env/<env>.json` and `packages/zambdas/.env/zambda-secrets-<env>.json` and restart the local zambda server. See the **M2M client must have a Practitioner profile** callout above for context. |
...
| --- | --- | --- |
| `packages/zambdas/.env/zambda-secrets-<env>.json` | Zambda runtime secrets — Oystehr IAM M2M client credentials, FHIR/Project API URLs, third-party service keys (SendGrid, Stripe, Candid, Anthropic, AdvAPACs, Google Cloud, …), feature flags. The `PROJECT_ID` here binds zambdas to a specific Oystehr project. | ✅ (`packages/zambdas/.gitignore`: `.env/*`) |
