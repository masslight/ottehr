/**
 * Manual test script for the ai-suggestion-notes zambda — procedure type only.
 *
 * Requires the local zambda server to be running on port 3000:
 *   npm run zambdas:start
 *
 * Usage:
 *   npx tsx scripts/test-procedure-suggestions.ts [--env local]
 */
...
import { TEST_SCENARIOS, TestScenario } from './test-procedure-suggestions-config';

const RUNS_PER_SCENARIO = 1;
const ZAMBDA_URL = 'http://localhost:3000/local/zambda/ai-suggestion-notes/execute';

const PROCEDURE_SCENARIOS = TEST_SCENARIOS;
...
async function main(): Promise<void> {
  console.log('Procedure Suggestions – Accuracy Check');
  ...
  for (const scenario of PROCEDURE_SCENARIOS) {
